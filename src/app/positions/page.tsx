        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <h1 className="text-3xl sm:text-4xl font-bold text-white">
                My <span className="text-amber-500">Positions</span>
              </h1>
            </div>
            <p className="text-zinc-500 max-w-md">
              Manage your liquidity positions across all Helios pools.{' '}
              <span className="text-amber-400">Rebalance to maximize efficiency!</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-800/50 border border-white/5">
              <Flame className="h-4 w-4 text-amber-500 animate-fire-flicker" />
    ...
    <button
      ...
      className={`
        ...
        ${disabled
          ...
          : highlight
            ? 'bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/30 hover:border-amber-500/50 hover-lift'
            : 'glass-card hover:border-amber-500/30 interactive-card'
        }
      `}
    >
      <div className={`
        inline-flex items-center justify-center p-2 rounded-lg mb-2
        ${highlight ? 'bg-amber-500/20 text-amber-500' : 'bg-zinc-700/50 text-zinc-400'}
      `}>
        {icon}
      </div>
      <div className="font-medium text-white text-sm">{title}</div>
      <div className={`text-xs ${highlight ? 'text-amber-400' : 'text-zinc-500'}`}>
        {description}
      </div>
    </button>
  )
